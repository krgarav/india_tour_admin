...
const TableFour = () => {
  ...
  const AllTours = tours.map((packageItem, key) => {
    const packagesItem =
      packageItem?.packages && !Array.isArray(packageItem.packages)
        ? packageItem.packages.packageDetails.map((item) => item.packageTitle)
        : ['No Package Involved'];
    return (
      <tr key={key}>
        <td className=" w-1/4 border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
          <h5 className="font-medium text-black dark:text-white">
            {packageItem.tourTitle}
          </h5>
          <p className="text-sm">₹{packageItem.tourPrice}/-</p>
        </td>
        <td className="border-b border-[#eee] py-5  dark:border-strokedark w-[55%]">
          <TextBoxes strings={packagesItem} />
        </td>
        <td className="w-1/4 border-b border-[#eee] py-5 px-4 dark:border-strokedark">
          <p
