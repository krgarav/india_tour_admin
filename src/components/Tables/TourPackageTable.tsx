...
const TourPackageTable = () => {
  ...
  const TourPackages = tours.map((packageItem, key) => {
    const packagesItem =
  ...
  const placeHolderJobs = new Array(10).fill(null).map((_, index) => (
    <tr key={index} className="bg-gray-200 text-left dark:bg-meta-4">
      ...
      <td className="min-w-[150px] py-4 px-4 font-medium text-black text-center dark:text-white">
        <Placeholder width="60%" height="1.5em" />
      </td>
    ...
    <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <div className="max-w-full overflow-x-auto">
        <table className="w-full table-auto">
          ...
          <tbody>
            {loading ? placeHolderJobs : TourPackages}
            {TourPackages.length === 0 && !loading && (
              <tr>
                <td
                  colSpan="100%"
                  style={{ textAlign: 'center', width: '100%' }}
                >
                  No Tour Packages present
                </td>
              </tr>
            )}
          </tbody>
